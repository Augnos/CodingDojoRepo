...
@app.route("/colors/<string:color>")
def show_color(color):

    print(f"Testing: The color is {color}")
    return f'<h2 style="background-color: {color}"> this is the color: {color}. I like the color {color}</h2>'


@app.route("/colors/<string:color>/<int:number>")
def color_multiplier(color, number):
    if (isinstance(number, int)):
        print(f'Duplicating {color} {number} times...')
        return f'<h2 style="background-color: {color}; text-align: center; margin: 0px auto; word-wrap: break-word">{color}</h2>' * number
