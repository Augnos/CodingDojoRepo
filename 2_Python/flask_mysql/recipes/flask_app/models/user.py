...
class User:
    ...
    def reg_validation(registration):
        ...
        if connectToMySQL('recipes_schema').query_db(query, registration):
            is_valid = False
    ...
    def login_validation(login):
        ...
        query = "SELECT email FROM users WHERE email = %(email)s;"
        if not connectToMySQL('recipes_schema').query_db(query, login):
        ...
        user_in_db = connectToMySQL('recipes_schema').query_db(query, login)
