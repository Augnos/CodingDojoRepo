...
function minSortedRotated(arr, low = 0, mid = Math.floor(arr.length / 2), high = arr.length - 1) {
    ...
    if (arr[low] > arr[mid]) return minSortedRotated(arr.slice(0, mid));
    if (arr[mid] > arr[high]) return minSortedRotated(arr.slice(mid, high + 1));
}

console.log(minSortedRotated(arr1));